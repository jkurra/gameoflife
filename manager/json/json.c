...
{
	//TODO: add checks and proper response if some value doesn't exist.
	if(model) {
...
void jsm_write_commons( commons_model *model, const char *pref_path  )
{
	...
	*/
	...
	//	gchar *theme = model->theme_path;
		...
		strings[5] = jsm_keypair("cellColor", cell, 1);
		strings[6] = jsm_keypair("defaultTheme", model->theme_path, 0);

		free(bgrn);
		free(cell);
		char *json = jsm_jobj(3, 7, strings);
